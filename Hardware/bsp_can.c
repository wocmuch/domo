...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *_hcan) {
    ...
    if (_hcan->Instance == CAN1) {
        ...
        case CAN_Motor7_ID:
        case CAN_Motor8_ID: {
//					i = CAN_RxHeader.StdId - CAN_2006motor1_ID;
//					motor_chassis[i].msg_cnt++ <= 50	?	get_motor_offset(&motor_chassis[i], Rx_Data) : get_motor_measure(&motor_chassis[i], Rx_Data);
    ...
    //else if (_hcan->Instance == CAN2)
        ...
        case CAN_Motor7_ID:
        case CAN_Motor8_ID: {
//					i = CAN_RxHeader.StdId - CAN_2006motor1_ID;//motor_chassis_dir
//					motor_chassis_dir[i].msg_cnt++ <= 50	?	get_motor_offset(&motor_chassis_dir[i], Rx_Data) : get_motor_measure(&motor_chassis_dir[i], Rx_Data);
            i = CAN_RxHeader.StdId - CAN_Motor1_ID;
            get_motor_measure(&motor_can2[i], Rx_Data);

//                if (motor_chassis_dir[i].msg_cnt++ <= 50) {
//                    get_motor_offset(&motor_chassis_dir[i], Rx_Data);
//                } else {
